...
class EnsembleModel(BaseModel):
    ...
    def fit(self, df: pd.DataFrame) -> "EnsembleModel":
        ...
        logger.info(
            f"[Ensemble] weights — Prophet={self._w_prophet:.3f}, ARIMA={self._w_arima:.3f}"
        )
    ...
    def predict(
        ...
        merged["forecast"] = (
            self._w_prophet * merged["forecast_p"]
            + self._w_arima * merged["forecast_a"]
        )
        merged["lower_bound"] = self._w_prophet * merged["lower_bound_p"].fillna(
            merged["forecast_p"]
        ) + self._w_arima * merged["lower_bound_a"].fillna(merged["forecast_a"])
        merged["upper_bound"] = self._w_prophet * merged["upper_bound_p"].fillna(
            merged["forecast_p"]
